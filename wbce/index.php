<?php
/**
 * WBCE CMS
 * Way Better Content Editing.
 * Visit https://wbce.org to learn more and to join the community.
 *
 * @copyright Ryan Djurovich (2004-2009)
 * @copyright WebsiteBaker Org. e.V. (2009-2015)
 * @copyright WBCE Project (2015-)
 * @license GNU GPL2 (or any later version)
 */

$starttime = array_sum(explode(" ", microtime())); 
// use $starttime for any type of benchmark testing you need

defined('WB_FRONTEND') or define('WB_FRONTEND', true);

// Include config file if already created on installation
if (file_exists($file = __DIR__ . '/config.php')) {
    require_once $file; unset($file);
}

if (!defined('TABLE_PREFIX')) {
    // If TABLE_PREFIX is not defined, WBCE CMS has not been installed yet.

    // Remark:  HTTP/1.1 requires a qualified URI incl. the scheme, name
    // of the host and absolute path as the argument of location. 
    $basePath    = rtrim(dirname($_SERVER['SCRIPT_NAME']), '/\\');
    $scheme      = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $redirectUrl = $scheme . '://' . $_SERVER['HTTP_HOST'] . $basePath . '/install/index.php';
    $statusLine  = $_SERVER['SERVER_PROTOCOL'] . ' 307 Temporary Redirect';
    header($statusLine);
    header('Location: ' . $redirectUrl);
    exit;
}

// Create new Frontend object.
$wb = new Frontend();

// Figure out which page to display. Stop processing if intro page was shown.
$wb->page_select() or die();

// Collect info about the currently viewed page and check permissions.
$wb->get_page_details();

// Collect general website settings.
$wb->get_website_settings();

// OPF hook, Load OutputFilter functions, needed at least for older versions of OpF.
if (is_readable($file = WB_PATH . '/modules/outputfilter_dashboard/functions.php')) {
    include $file; unset($file);
    // Use 'cache' instead of 'nocache' to enable page-cache.
    // Do not use 'cache' in case you use dynamic contents (e.g. snippets)!
    opf_controller('init', 'nocache');
}

// Load frontend functions
require WB_PATH . '/framework/frontend.functions.php';

// Get template include file if exits
if (is_readable($file = WB_PATH . '/templates/' . TEMPLATE . '/include.php')) {
    include $file; unset($file);
}

// Fetch output generated by core, preinit, init, snippets...
$preOutput = ob_get_clean();

//Get page content in buffer for OutputFilter processing
ob_start();
require WB_PATH . '/templates/' . TEMPLATE . '/index.php';

// Fetch the content for applying filters to it
$bufferedContent = ob_get_clean();

// Append the PRe output  so filter can handle it
$content = $bufferedContent . $preOutput;

// Load OPF Dashboard OutputFilter functions
if (is_readable($file = WB_PATH . '/modules/outputfilter_dashboard/functions.php')) {
    require_once($file);  unset($file);
    // Now apply pageType OutputFilters to the whole content
    if (function_exists('opf_apply_filters')) {
        $content = opf_controller('page', $content);
    }
}

// Process Direct Output if set. 
// This clears buffered contents and 
// ends the script here before echo $content is reached.
$wb->sendDirectOutput();

// Now, send complete page content to the browser
echo $content;
exit;
