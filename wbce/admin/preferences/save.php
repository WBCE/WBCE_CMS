...
function save_preferences(&$admin, &$database)
{
    ...
    $sRePassword      = (is_null($sRePassword) ? '' : $sRePassword);
    ...
    if ($admin->doPasswordEncode($sCurrentPassword) != $database->get_one($sSql)) {
        ...
        if (sizeof($aErrMsg) == 0) {
            ...
            if ($sPwHashNew != '') $aUpdate['password'] = $database->escapeString($sPwHashNew);            
            if ($email != '')      $aUpdate['email']    = $database->escapeString($email);
    ...
    return ( (sizeof($aErrMsg) > 0) ? implode('<br />', $aErrMsg) : '' );
}
