...
$results = $database->query("SELECT user_id FROM {TP}users WHERE email = '".$admin->add_slashes($_POST['email'])."' AND user_id <> '".$user_id."' ");
...
if($sNewPassword != ''){
    ...
    if (is_array($checkPassword)){
        $admin->print_error(explode('<br />', $checkPassword), $js_back);
    } else {
