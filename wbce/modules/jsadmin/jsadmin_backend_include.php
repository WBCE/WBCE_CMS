...
if($page_type != '') {
	...
	if($page_type == 'modules') {
		if(!get_setting('mod_jsadmin_persist_order', '1')) {
	...
	if(in_array('dragdrop.js', $js_scripts)) {
		I::insertJsFile(WB_URL.'/include/jquery/jquery-ui-min.js', 'BODY BTM-');
	}

	// Load jsadmin JS files
	foreach($js_scripts as $script) {
		I::insertJsFile($JSADMIN_PATH."/js/".$script, 'BODY BTM-');
	}

	// Load jsadmin CSS (pages + sections)
	if(in_array($page_type, array('pages', 'sections'))) {
		I::insertCssFile($JSADMIN_PATH.'/backend.css', 'HEAD');
	}
}