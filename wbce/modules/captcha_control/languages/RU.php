...
// ── No HTTPS fallback ────────────────────────────────────────────────────────
$CAPTCHA['NO_HTTPS_HEADING']       = 'Настройки недоступны — требуется HTTPS';
$CAPTCHA['NO_HTTPS_INFO']          = 'ALTCHA использует функцию браузера (Web Crypto), которая работает только на сайтах с HTTPS. Этот сайт в настоящее время доступен только по обычному HTTP, из-за чего виджет никогда не сможет завершить проверку — поэтому здесь нечего настраивать, пока не будет установлен SSL-сертификат и сайт не будет переведён на HTTPS.';
$CAPTCHA['NO_HTTPS_FALLBACK']      = 'Тем временем автоматически используется простая математическая капча (например, «3 + 5 = ?»), чтобы формы оставались защищёнными.';
$CAPTCHA['VERIFICATION_INFO_RES']  = 'Пожалуйста, решите пример, чтобы подтвердить, что вы не робот.';
