...
function clone_page($title,$parent,$pagetoclone,$copy_title,$visibility) {
  ...
  if($copy_title) {
	...
	while (false != ($is_section=$get_section->fetchRow( MYSQL_ASSOC ))) {
		...
		// fetch tables matching $module type (TABLE_PREFIX limits results to tables of this WBCE session)
		$query = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME LIKE '". TABLE_PREFIX . "mod_".$module."%'";
