...
function nestedPagesArray($iParent = 0) {	
	...
	$aPages = array(); 
	if(!$oPages) return $aPages; // no pages in DB yet; return empty array
	$refs = array();
	$aQueryKeys = array(
	...
	while($p = $oPages->fetchRow(MYSQL_ASSOC)) {
		...
		foreach($admin->get_groups_id() as $cur_gid) {
			if (in_array($cur_gid, $aAdminGroups)) 	$in_group = true;
		}			
...
function parentPageList($nestedPagesArray, $iCurrentPageId = 0, $bAllLevels = false) {
	...
	foreach($nestedPagesArray as $p) { 
		...
		if ($iCurrentPageId != 0 && $p['parent'] == $iCurrentPageId)  continue;
		if ($p['visibility'] == 'deleted' && $sPageTrash != 'inline') continue;
		...
		if($p['level'] + 1 < PAGE_LEVEL_LIMIT || $bAllLevels == true) {						
			...
			if(defined('DB_CHARSET') && DB_CHARSET == 'utf8'){				
				$sSpacer = str_repeat(($p['level'] == 1 ? "&nbsp;" : "&nbsp;&nbsp;"),
					(($p['level']))).(($p['level'] > 0) ? ($p['canMoveDOWN'] ? ' ├─ ' : ' └─ ') : ""
				); 
				$sSpacer = str_repeat("".(($p['canMoveDOWN'] || $p['canMoveUP']) && $p['level'] != 0 ? '&nbsp;·&nbsp;' : '&nbsp;'),
					(($p['level']))).(($p['level'] > 0) ? ($p['canMoveDOWN'] ? ' ├─ ' : ' └─ ') : " "
				...
				$sSpacer = str_repeat("&nbsp;",
					(($p['level'])*2*2)).(($p['level'] > 0) ? ($p['canMoveDOWN'] ? ' |-- ' : " '-- ") : ""
				); 
			}                        
