...
if ($wb->is_authenticated()) {
	...
	//So koennte dann der Edit-Link dann unten im Template aussehen:
...
// Üblich ist aber eine bestimmte Aufteilung. Weiter unten geben wir diese Bloecke aus, und das Layout aendert sich, je nachdem, ob die Bloecke auch Inhalt haben.

// Auch die Bloecke laden wir gleich hier in eine Variable $contentblock (Array), das hat Vorteile 
// weil das Template so wesentlich übersichtlicher ist :
...
foreach($block as $k=>$v){ //und haengen in einer Schleife alle an.
    ...
    $contentblock[$k]=page_content($k, false); // don't echo but return page content 
}
...
// Damit ist der PHP Teil zu Ende 
...
// Achtung zwischen dem ende des PHP Bereichs und der Doctype definition sollte kein leerzeichen oder return sein. 
?><!DOCTYPE html>
...
<!--(PH) JS HEAD TOP+ -->
    ...
    <?phpregister_frontend_modfiles('jquery');?>
    <!-- Hier wird das JS dazugeladen, was Module und Templates brauchen. -->
	<?phpregister_frontend_modfiles('js');?>
<!--(PH) JS HEAD TOP- -->
...
<?php 
    ...
    echo '    <!--(MOVE) META HEAD- --><meta property="og:image" content="'.OG_IMAGE.'"/><!--(END)-->';}
?>
