 ...
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * Legacy compatibility stub for class.order.php
 * ─────────────────────────────────────────────────────────────────────────────
 * 
 *
 ...
 * Do NOT manually require this file or Order.php in new code.
 */

defined('WB_PATH') or die('No direct access allowed');

// Load the modern Order class ONLY if it hasn't been loaded yet
// (The autoloader may have already loaded it)
if (!class_exists('Order')) {
    require_once WB_PATH . '/framework/Order.php';
}

// Create alias so old code like `new order()` or `$GLOBALS['order']` patterns still work
if (class_exists('Order') && !class_exists('order', false)) {
    class_alias('Order', 'order');
}
...
if (defined('WBCE_CANONICAL_DEBUG') && WBCE_CANONICAL_DEBUG) {
    trigger_error(
        'require_once WB_PATH . \'/framework/class.order.php\' is deprecated. ' .
