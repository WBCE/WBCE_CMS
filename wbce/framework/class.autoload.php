...
class WbAuto
{
     ...
     */
    public static function AddFile($ClassName = "", $ClassFile = "", $Overwrite = false, $DirektPath = false)
    {

        //Check for valid call return error if invalid
        if ($ClassName == "") {
            return ("AddFile, no 'ClassName' set!");
        }
        if ($ClassFile == "") {
            return ("AddFile, no 'ClassFile' set!");
        }
        if (!is_string($ClassName)) {
            return ("AddFile,'ClassName' is not a string");
        }
        if (!is_string($ClassFile)) {
            return ("AddFile,'ClassFile' is not a string");
        }
        if (!is_bool($Overwrite)) {
            return ("AddFile invalid parameter 'Overwrite' is not a boolean! ");
        }
        if (!is_bool($DirektPath)) {
            return ("AddFile invalid parameter 'DirektPath' is not a boolean! ");
        }
        if (isset(WbAuto::$Files[$ClassName]) and $Overwrite == false) {
            return ("Class already exists ($ClassName)");
        }
        ...
        if (!is_file($LoadFile)) {
            return ("AddFile, File does not exist or not a file: $LoadFile");
        }
        if (!is_readable($LoadFile)) {
            return ("AddFile, File not readable: $LoadFile");
        }
