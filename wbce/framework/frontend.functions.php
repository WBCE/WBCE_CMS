...
if (!function_exists('block_contents')) {
    ...
    function block_contents($uBlock = 1){
        ...
        if (!defined('PAGE_CONTENT')) {
            ...
            // First get all sections for this page			
            $sSql = 'SELECT `section_id` FROM `{TP}sections` WHERE `page_id`=%d AND `block`=%d ORDER BY `position`';

            if (!($rSections = $database->query(sprintf($sSql, $wb->page_id, $iBlockID)))) {
                return; // return if no results
            }
            ...
            if ($rSections->numRows() == 0) {
                ...
                if (!($rSections = $database->query(sprintf($sSql, $page_id, $iBlockID)))) {
                    return;
                }
                // Still no cotent found? Give it up, there's just nothing to show!
                if ($rSections->numRows() == 0) {
            ...
            // Loop through sections and include their view.php file
            while ($row = $rSections->fetchRow()) {
                $aSection = get_section_array($row['section_id']);
                // skip this section if it is out of publication-date
                $now = time();
                if (!(
                        ($now <= $aSection['publ_end'] || $aSection['publ_end'] == 0) 
                ...
                // check if module exists
                if (file_exists(WB_PATH . '/modules/' . $aSection['module'] . '/view.php')) {
                    ob_start(); // fetch original content
                    $section_id = $aSection['section_id'];
                    require WB_PATH . '/modules/' . $aSection['module'] . '/view.php';
                    $content = ob_get_clean();

                    //OPF hook
                    if(function_exists('opf_apply_filters')) {
                        $content = opf_controller('section', $content, $aSection['module'], $page_id, $aSection['section_id']);
                    }
                } else {
                    continue;
                }
                ...
                if (isset($_GET['searchresult']) && is_numeric($_GET['searchresult']) && !isset($_GET['nohighlight']) && isset($_GET['sstring']) && !empty($_GET['sstring'])) {
                    ...
                    $aBlockSections[]['content'] = search_highlight($content, $arr_string);
                } else {
                    ...
                    $aBlockSections[$section_id]['position']   = $aSection['position'];
                    $aBlockSections[$section_id]['section_id'] = $aSection['section_id'];
                    $aBlockSections[$section_id]['module']     = $aSection['module'];
                    $aBlockSections[$section_id]['evenodd']    = ($aSection['position'] %2 == 1) ? 'even' : 'odd';
                    $aBlockSections[$section_id]['sec_anchor'] = $sec_anchor;
                    $aBlockSections[$section_id]['content']    = $content;
                }                                  
            }
        } else {
            ob_start();
...
if (!function_exists('page_content')) {
    function page_content($iBlockID = 1, $bEcho = 1){
        $sRetVal = '';
        if (!defined('PAGE_CONTENT')) {
            foreach(block_contents($iBlockID) as $section){
                if ($section['sec_anchor'] != ''){
                        $sRetVal .= PHP_EOL . $section['sec_anchor'] . PHP_EOL . $section['content'];
                } else {
                        $sRetVal .= PHP_EOL . $section['content'];
                }
            }
        } else {
            ob_start();
